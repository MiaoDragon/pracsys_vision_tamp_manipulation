...
def construct_occlusion_graph(obj_ids, obj_pybullet_poses, camera, pid):
    ...
    for obj_i in range(len(obj_ids)):
        ...
        for obj_j in range(len(obj_ids)):
            ...
            if mask.sum() == 0:
                continue
            indices = mask & (depth_imgs[obj_j] < min_depth)
            min_id_img[indices] = obj_ids[obj_j]
            min_depth[indices] = depth_imgs[obj_j][indices]
    ...
    for i in range(len(nodes)):
        pose_pos = (-8 * obj_pybullet_poses[i][1, 3], 8 * obj_pybullet_poses[i][0, 3])
        dot.add_node(nodes[i], pos='%f,%f!' % pose_pos)
        node_poses.append(pose_pos)
    for i in range(len(edges)):
        dot.add_edge(edges[i][0], edges[i][1])

    cam_pose = (-8 * camera.info['extrinsics'][1, 3], 8 * camera.info['extrinsics'][0, 3])
    dot.add_node('cam', pos='%f,%f!' % cam_pose)
...
def update_occlusion_graph(
    ...
    for i in range(len(nodes)):
        ...
        dot.add_node(
            nodes[i],
            pos='%f,%f!' %
            (-8 * obj_pybullet_poses[i][1, 3], 8 * obj_pybullet_poses[i][0, 3]),
            fillcolor=color,
            style='filled'
        )
    for i in range(len(edges)):
    ...
    dot.add_node(
        'cam',
        pos='%f,%f!' %
        (-8 * camera.info['extrinsics'][1, 3], 8 * camera.info['extrinsics'][0, 3])
    )
...
def update_occlusion_graph_color(
    nodes,
    node_poses,
    edges,
    cam_pose,
    ...
    for i in range(len(nodes)):
        ...
        dot.add_node(
            nodes[i],
            pos='%f,%f!' % (node_poses[i][0], node_poses[i][1]),
            fillcolor=color,
            style='filled'
        )
    for i in range(len(edges)):
